...
RETURN_CODE
Token::speech_get_token(const char *api_key, const char *secret_key, const char *scope, char *token) {
...
RETURN_CODE Token::parse_token(const char *response, const char *scope, char *token) {
    //  ====  获取 token字段 =========
...
RETURN_CODE Token::obtain_json_str(const char *json, const char *key, char *value, int value_size) {
    int len = 4 + strlen(key) + 1;
    char search_key[len];
    snprintf(search_key, len, "\"%s\":\"", key);
    const char *start = strstr(json, search_key);
    if (start == nullptr) { // 没有找到
